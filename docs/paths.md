...
| **Raft** | 3 plank + 1 rope (+1 barrel) | Waterline | A real deck: gunwales, push pole, climb-aboard. Look down (or hold Dive) + walk to pole; look level to work. |
| **Rig sail** | 1 plank + 1 canvas + 1 rope | On the raft | Mast, canvas, and a stern tiller — slow trade-wind drift while you're aboard. Stand aft to helm. A gale can tear it; **Mend** with canvas + rope. |
...
Fall off and **Climb** the raft from the water (F when near). Kindle a fire on the deck if you want heat under sail. Deck fires ride with the raft. An empty raft keeps her heading — she won't spin to chase leftover pole speed. **Look down** (or hold Dive / Shift) anywhere aboard and walk to **pole**; look level to walk and work fittings. A quiet status line confirms when you're poling, at the helm, beached, or anchored. On sand she sits still — **Shove** clear of the shelf, then pole again. **Haul** her up the beach when you mean to camp. A gale can tear the sail — **Mend** it. **Lash Deck** up to five times for a ferry-sized platform.
